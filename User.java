		...
		if(ae.getSource() == b4) // Ok is clicked
		{
			...
			else
			{
				try {
				    ...
				    conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/agro_services",
				            "root", "password");
						...
						tf1.addKeyListener(new KeyAdapter() 
						{
				        	public void keyPressed(KeyEvent EVT) 
				        	{
								...
								try {
								    ...
								    qry = "SELECT User_Name,User_Password,User_Type,User_ID FROM Login WHERE User_ID=?";
								    PreparedStatement Record = conn.prepareStatement(qry);
								    Record.setInt(1, ID);
								    ResultSet rs = Record.executeQuery(); // execute query
								    if (rs.next()) { 
								        ...
								        String msg = "There is no User to display.";
								        JOptionPane.showMessageDialog((Component)null,msg,"Agro Services", JOptionPane.ERROR_MESSAGE);
								        tf1.requestFocus();
								    }
								    conn.close();
								} catch(SQLException sqle) {
 ...
 // End of catch			
	...
	void display_Record(int flag)
	{
		...
		try {
		    ...
		    String qry = "";
		    if (flag == 1) {
		        // Query for selecting the first record
		        qry = "SELECT Login.*, Login.User_ID FROM Login WHERE Login.User_ID = (SELECT MIN(Login.User_ID) AS Expr1 FROM Login);";
		    } else if (flag == 2) {
		        // Query for selecting the previous record
		        qry = "SELECT Login.*, Login.User_ID FROM Login WHERE Login.User_ID < ? ORDER BY Login.User_ID DESC LIMIT 1;";
		    } else if (flag == 3) {
		        // Query for selecting the next record
		        qry = "SELECT Login.*, Login.User_ID FROM Login WHERE Login.User_ID > ? ORDER BY Login.User_ID LIMIT 1;";
		    } else if (flag == 4) {
		        // Query for selecting the last record
		        qry = "SELECT Login.*, Login.User_ID FROM Login WHERE Login.User_ID = (SELECT MAX(Login.User_ID) AS Expr1 FROM Login);";
		    }
