 	  ...
 	  if(e.getSource()==b1)
 	   {
			...
			try {
				...
				String qry = "SELECT User_Password,User_Type,User_ID FROM Login WHERE User_Name=?";
				PreparedStatement Check_User = conn.prepareStatement(qry);
				Check_User.setString(1, usernm);
				ResultSet rs = Check_User.executeQuery(); // execute query
				rs.next();
				int i = rs.getRow();
				if (i == 0) { // User Does not exist
					String msg = "User "+"'"+usernm+"'"+" dose not exist.";
					int j = JOptionPane.showConfirmDialog(this, msg, "Login : Agro services", JOptionPane.OK_CANCEL_OPTION, JOptionPane.WARNING_MESSAGE);
				...
				} else if (i > 0) { // User exists
					String UserPass = rs.getString(1);
					if (pswd.equalsIgnoreCase(UserPass)) {
						// Password Match grant Access.
						String msg = "Successful Login.";
						int j = JOptionPane.showConfirmDialog(this, msg, "Login : Agro Services", JOptionPane.DEFAULT_OPTION, JOptionPane.INFORMATION_MESSAGE);
						if (j == JOptionPane.OK_OPTION) {                    
